...
# the syntax here is 'piping'

#piping
gapminder %>% head() -> gap.10
gapminder %>% nrow()
gapminder %>% filter(country == "France")
gapminder %>% filter(lifeExp < 30) # filter pulls out rows that meet certain criteria
gapminder %>% filter(lifeExp < 30) %>% select(lifeExp)
gapminder %>% select(year:gdpPercap)


# filter

#piping an object into a function
# same as
gap.10 <- head(gapminder)
...
gapminder %>% 
  ...
  do(tidy(lm(lifeExp ~ gdpPercap, data = .), conf.int = TRUE)) %>% 
	...
	ggplot(aes(y = estimate, x = factor(country))) + geom_point() + 
	geom_errorbar(aes(ymin = conf.low, ymax = conf.high))
ggsave("coef.plot.png")
mod1
```
## do applies any function to the grouped dataset
